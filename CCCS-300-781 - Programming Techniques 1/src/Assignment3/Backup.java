					...
					if (answer.equals("Y") || answer.equals("y")) {  // if choice is "Y" or "y"
						...
						if (answer.equals("N") || answer.equals("n")) { // Option chosen is to enter a letter at a time
							...
							System.out.print("Letter(s) available: ");
								...
								while (!enterAletter.hasNext("[A-Za-z]")) { // Check that user input are only characters
									System.out.println("Error: try again! A letter only...");
									enterAletter.next().toUpperCase();
								}
								
								String z = enterAletter.next().toUpperCase();
								char b = z.charAt(0); // Convert String from Scanner input to char for comparison with char[] arrays
