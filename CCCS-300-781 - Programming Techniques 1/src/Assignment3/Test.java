					...
					if (answer.equals("Y") || answer.equals("y")) {  // if choice is "Y" or "y"
						...
						if (answer.equals("N") || answer.equals("n")) { // Option chosen is to enter a letter at a time
							...
							System.out.print("Letter(s) available: ");
								...
								char checkLetter = ' '; // char checkLetter will be used as main variable to check and counter entry
								String z = "";
								
								/* Check if letter entered has already been used or if entry is other that a letter */
								while (letterCount == 0 && z != ("[A-Za-z]")) { // Check that user input are only characters
									...
									if (letterCount == 0)
										System.out.println("Error: letter already used or you have not entered a letter. Try again!");
								}
								
								/* Check if letter has already been used */
								for (int i = 0; i < alphabet.length; i++) {
									
									if (alphabet[i] == checkLetter)
										letterCount++;
								}
